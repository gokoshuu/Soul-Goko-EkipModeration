const { Client, Collection, DiscordAPIError, Intents } = require("discord.js");
...
const ms = require("ms"); 
const tag = []
let server = serverSettings.findOne({
   
});
class CanBot extends Client {
  constructor(options) {
    super(options);
  ...
  async kaldır(arr, value) {
    ...
    async yolla(mesaj, msg, kanal) {
      ...
      const embd = new Discord.MessageEmbed()
        .setAuthor({ name: msg.tag, iconURL: msg.displayAvatarURL({ dynamic: true }) })
        .setColor("RANDOM")
  ...
  async punishPoint(userID) {
    ...
    let reklam = filterArr.filter(x => x == "Cezalı - Reklam").length || 0
    let point = (chatMute * 8) + (voiceMute * 10) + (jail * 15) + (ban * 20) + (uyarı * 3) + (reklam * 20)
    return point
  }
...
const init = async () => {
  ...
  const evtFilesx = await readdir("./src/functions/");
  ...
  evtFilesx.forEach(file => {
    ...
    const eventz = new (require(`./src/functions/${file}`))(client);
    client.on(eventNamez, (...args) => eventz.run(...args));
    delete require.cache[require.resolve(`./src/functions/${file}`)];
  });
